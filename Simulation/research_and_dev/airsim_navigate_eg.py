...
while True:
	...
	# response, = client.simGetImages([ImageRequest("0", airsim.ImageType.Scene, False, False)])
	if (result == "\0"):
		if (not help):
			help = True
		...
		rawImage = np.fromstring(result.image_data_uint8, dtype=np.uint8)
		# rawImage = np.frombuffer()(result, np.int8)
		# rawImage = np.frombuffer(result.image_data_uint8, dtype=np.uint8).reshape(result.height, result.width, 3)
