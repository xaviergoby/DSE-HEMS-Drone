...
#        self.client.hoverAsync().join()
        ...
        self.client.moveToPositionAsync(-10, 75, -15, 5)
        ...
        for i in range(1,11):
            ...
            if (len(lidarData.point_cloud) < 3):
                ...
                points,distances = self.parse_lidarData(lidarData)
...
if __name__ == "__main__":
    ...
    total_scans = lidarTest.scans[0]
    for scan in lidarTest.scans[1::]:
        total_scans = np.vstack((total_scans,scan))
    np.savetxt('coords.txt',np.transpose(total_scans))
    
    total_distances = lidarTest.distance_data[0]
    for distances in lidarTest.distance_data[1::]:
